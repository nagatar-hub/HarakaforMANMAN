-- Store-aware operator exclusions by normalized Excel product ID plus a
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.excel_product_mapping (
      ...
      v_store, v_item.franchise, v_item.excel_product_id, NULL, 'disabled', NULL,
  ...
  IF v_import.status = 'confirmed' THEN
    ...
    IF v_matched = 0 THEN
      RAISE EXCEPTION 'order_list_import has no matched items: %', p_import_id
        USING ERRCODE = '22023';
    END IF;
    IF EXISTS (SELECT 1 FROM public.run WHERE store = v_import.store AND status = 'running') THEN
...
DECLARE
  v_import public.order_list_import%ROWTYPE;
  v_requested_store TEXT;
  v_run_id UUID;
  v_run_status TEXT;
  v_request_run_import_id UUID;
  v_request_run_store TEXT;
...
BEGIN
  ...
  SELECT store INTO v_requested_store
  FROM public.order_list_import
  WHERE id = p_import_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'order_list_import not found: %', p_import_id
  ...
  SELECT sync_run.id, sync_run.status, sync_run.order_list_import_id, sync_run.store,
         sync_run.order_list_sync_request_fingerprint, request_import.status
  INTO v_run_id, v_run_status, v_request_run_import_id, v_request_run_store,
  ...
  IF FOUND THEN
    IF v_request_run_import_id IS DISTINCT FROM p_import_id THEN
      RAISE EXCEPTION 'order-list sync request_id belongs to another import: %',
        ...
        USING ERRCODE = '55000';
    END IF;
    IF v_request_run_store IS DISTINCT FROM v_requested_store THEN
      RAISE EXCEPTION 'order-list sync request_id belongs to another store: %', p_request_id
        USING ERRCODE = '55000';
    END IF;
  ...
  SELECT id, status, order_list_import_id, store,
  ...
  WHERE target_import.id = p_import_id
    ...
    AND EXISTS (
      ...
      WHERE active_run.id = p_run_id
        ...
        AND active_run.store = target_import.store
...
BEGIN
  SELECT store, order_list_sync_request_id, order_list_sync_request_fingerprint
  INTO v_store, v_request_id, v_request_fingerprint
  FROM public.order_list_import
  WHERE id = p_import_id
  FOR UPDATE;
  IF NOT FOUND THEN
  ...
  WHERE id = p_run_id
    ...
    AND order_list_sync_request_id IS NOT DISTINCT FROM v_request_id
    AND order_list_sync_request_fingerprint IS NOT DISTINCT FROM v_request_fingerprint
    AND status = 'running';
  IF NOT FOUND THEN
...
-- Preserve legacy RPC signatures while routing every write through the
-- store-aware batch resolver above.
...
BEGIN
  SELECT status, store INTO v_import_status, v_store
  FROM public.order_list_import
  WHERE id = p_import_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'order_list_import not found: %', p_import_id
  ...
  SELECT * INTO v_item
  FROM public.order_list_item
  WHERE id = p_item_id AND import_id = p_import_id;
  SELECT * INTO v_mapping
  FROM public.excel_product_mapping
  WHERE id = v_item.mapping_id AND store = v_store;
  SELECT * INTO v_db_card
  FROM public.db_card
  WHERE id = v_item.db_card_id AND store = v_store;

  IF v_item.id IS NULL OR v_mapping.id IS NULL OR v_db_card.id IS NULL THEN
    RAISE EXCEPTION 'store-scoped mapping result is incomplete: %', p_item_id
      USING ERRCODE = '55000';
  END IF;
...
DECLARE
  ...
  v_results JSONB;
BEGIN
  v_result := public.resolve_order_list_item_selections(
    p_import_id,
    p_mappings,
    '[]'::JSONB,
    p_allow_unresolved
  ...
  SELECT coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'applies_from_next_import', imported.status = 'applied'
  ...
  JOIN public.order_list_import AS imported
    ON imported.id = p_import_id
  JOIN public.order_list_item AS item
    ON item.id = (selection.value ->> 'item_id')::UUID
   AND item.import_id = imported.id
  JOIN public.excel_product_mapping AS mapping
    ON mapping.id = item.mapping_id
   AND mapping.store = imported.store
  JOIN public.db_card AS card
    ON card.id = item.db_card_id
   AND card.store = imported.store;

  RETURN v_result || jsonb_build_object('results', v_results);
END;
$$;
...
-- Failure completion is fenced by both the store and the request lease. A
-- recovered stale worker cannot fail a successor run for the same import.
...
DECLARE
  ...
  v_run_status TEXT;
BEGIN
  SELECT status, store, order_list_sync_request_id,
         order_list_sync_request_fingerprint
  INTO v_import_status, v_store, v_request_id, v_request_fingerprint
  FROM public.order_list_import
  WHERE id = p_import_id
  FOR UPDATE;
  IF NOT FOUND THEN
  ...
  FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'run not found for store/import lease: % / % / %',
      v_store, p_run_id, p_import_id
      USING ERRCODE = 'P0002';
  END IF;
  ...
  WHERE id = p_import_id
    ...
    AND order_list_sync_request_fingerprint IS NOT DISTINCT FROM v_request_fingerprint;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'order_list_import lease changed while failing: %', p_import_id
      USING ERRCODE = '55000';
  END IF;
END;
$$;
