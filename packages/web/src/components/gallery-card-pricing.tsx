type GalleryPricing = {
  ...
  comparison?: {
    rows: { store: string; rawPrice: number; highRate: number; highPrice: number;
      selectedHigh: boolean; excluded?: boolean }[];
    ...
    <div className="overflow-x-auto">
      <table className="border-separate border-spacing-y-1 text-left">
        ...
        <tbody>{pricing.comparison.rows.map(row => <tr key={row.store} className={row.excluded ? 'opacity-60' : undefined}>
          ...
          <td className="pl-2 tabular-nums">{priceText(row.highPrice)}{row.selectedHigh ? '（採用）' : ''}</td>
        </tr>)}</tbody>
      </table>
    </div>
    <p className="text-text-primary">採用：{pricing.comparison.high.store}{' '}
