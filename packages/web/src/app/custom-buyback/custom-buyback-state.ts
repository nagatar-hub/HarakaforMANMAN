...
export function customBuybackCsv(items: CustomBuybackItemRow[]): string {
  const shinsoku = items.some(item => item.price_source === 'shinsoku');
  const rows: Array<Array<string | number | null>> = [
    ['位置', '商品名', 'グレード', 'リスト番号', 'レアリティ', '元価格', '表示価格', '募集数', shinsoku ? '価格取得元' : '最高価格店舗', '手修正理由', '価格基準日', '買取Checker商品ID', 'Excel商品ID', ...(shinsoku ? ['Shinsoku商品ID'] : [])],
    ...items.map((item) => [
      ...
      item.override_reason, item.price_source_date, item.source_kaitori_product_id, item.excel_product_id, ...(shinsoku ? [item.source_shinsoku_id ?? null] : []),
    ]),
  ];
