...
type BatchResult = PromiseLike<{ error: { message: string } | null }>;
type DynamicTable = {
  insert(rows: Record<string, unknown>[]): BatchResult;
  upsert(rows: Record<string, unknown>[], options: { onConflict: string }): BatchResult;
};
