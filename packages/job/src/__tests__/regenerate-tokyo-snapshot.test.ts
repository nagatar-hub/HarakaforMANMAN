...
const mockLoadSettings: jest.Mock = jest.fn(async () => ({}));
...
jest.mock('../lib/buyback-sheet', () => ({ publishManmanBuybackSheet: (...args: unknown[]) => mockPublish(...args) }));
...
test('Tokyo normal snapshot regeneration preserves stored prices, exact BOX assets and one price without Sheets writes', async () => {
  ...
  try {
    for (const [store, snapshot, legacy] of [
              ...
              : table === 'order_list_import' ? { business_date: '2026-09-07' }
                : table === 'asset_profile' ? [{ layout_config: layout, total_slots: 30, template_box_storage_path: 'wrong-legacy-template' }]
                  : table === 'layout_template' ? { id: 'box-layout', franchise: 'Pokemon', slug: 'box_30', grid_cols: 6, total_slots: 30,
                    template_storage_path: 'tokyo/box_30.png', card_back_storage_path: 'tokyo/box-back.png', layout_config: layout } : null, error: null });
      ...
      expect(mockLoadSettings).toHaveBeenCalledTimes(legacy ? 1 : 0);
      expect(mockPublish).toHaveBeenCalledTimes(legacy ? 1 : 0);
      const rendered = (mockCompose.mock.calls as unknown[][])[0][0] as Record<string, any>;
      expect(rendered.cards[0].price_high).toBe(9300);
      expect(rendered.skipPriceLow).toBe(!legacy);
